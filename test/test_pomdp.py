...
def test_pomdp_initialization():
    ...
    motion_model = VelocityIntegratorModel(
        i_x=50.0, i_y=28.0, dt=0.1, max_v=30)
...
# 2. Transition model
def test_cartesian_transition(pomdp):
    ...
    T_matrix = pomdp.T_cartesian(X_n, u)
    end_time = time.time()
...
def test_T_cartesian_indexing_validation(pomdp):
    """Validate that row i corresponds to T(\cdot | X_n[i]) and column j to next state X_n[j]."""
    print("\n=== Validating T_cartesian Indexing (rows: current i, cols: next j) ===")
    ...
    assert np.allclose(
        T_vec, T_loop, atol=1e-10), "Row/column indexing mismatch in T_cartesian"
...
def demonstrate_T_cartesian_broadcasting(pomdp):
    """Demonstrate the broadcasting steps used to build T_cartesian with a tiny example."""
    print("\n=== Demonstrating T_cartesian Broadcasting Shapes ===")
...
def test_T_cartesian_entropy_vs_sigma(pomdp):
    """Sweep σ_w and report average row entropy to show determinism vs spread."""
    print("\n=== T_cartesian Row Entropy vs σ_w ===")
    X_n = SquareLatticeQuantizer(0, 3, 0, 3, n=4).get_quantized_points()
    u = np.array([0.4, 0.4])
...
def test_T_cartesian_outside_state_space(pomdp):
    """Probe behavior when predicted state lies outside the convex hull of X_n."""
    print("\n=== T_cartesian Outside State Space Behavior ===")
...
def test_visualize_ogm_and_raycasting_hits(pomdp):
    ...
    grid_data = pomdp.map.occupancy_map.data.T

    # y_star from ray_casting on current OGM
    y_star = pomdp.ray_casting(
        robot_pose[np.newaxis, :], pomdp.map.occupancy_map.data)[0]
    angles = pomdp.sensor.angles
    ...
    ax.imshow(grid_data, cmap='gray_r', origin='lower',
...
def main():
    ...
    pomdp = test_pomdp_initialization()

    # 2. Test transition matrix functionality
    # test_cartesian_transition(pomdp)
    # test_T_cartesian_indexing_validation(pomdp)
    # demonstrate_T_cartesian_broadcasting(pomdp)
    # test_T_cartesian_entropy_vs_sigma(pomdp)
    # test_T_cartesian_outside_state_space(pomdp)
