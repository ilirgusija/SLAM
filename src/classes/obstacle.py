...
class Obstacle():
    ...
    def update(self, pos=None, recycle_pos=True):
        ...
        """
        if pos is None:
            disp_x = self.centroid[0] + self.vel[0] * self.time + \
                0.5 * self.acc[0] * (self.time**2)  # s_x = ut + 0.5at^2
            disp_y = self.centroid[1] + self.vel[1] * self.time + \
                0.5 * self.acc[1] * (self.time**2)  # s_y = ut + 0.5at^2
        else:
