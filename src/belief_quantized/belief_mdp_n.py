...
from ..classes.mapping import BaseMap
from ..classes.model import LIDAR, RangeBearingSensor, SingleIntegratorModel, DoubleIntegratorModel
from ..classes.obstacle import Obstacle
from ..classes.quantizer import StateQuantizer, ActionQuantizer, ObservationQuantizer
from abc import ABC, abstractmethod
from typing import Any, Literal
from ..classes.pomdp import SLAM_POMDP, Localization_POMDP, Mapping_POMDP
try:
...
# Import logsumexp and entr from appropriate backend
if is_cupy:
    from cupyx.scipy.special import logsumexp, entr
...
class BaseBeliefMDP_n(ABC):
    ...
    Belief-MDP is defined as a four tuple (B_n, U_n, η, ρ_n).
    ...
    def _init_common(
        ...
        """
        self.n = n
        # n_map: quantization level of the map's spatial grid (e.g. LandmarkMap.n).
        # Separate from n (robot pose quantization) so both can vary independently.
        self.n_map = getattr(_map, 'n', None)
        if obs_n is None:
            obs_n = n
        if action_n is None:
            action_n = n
        self.obs_quantization_mode = 'euclidean'
        ...
        self._configure_observation_quantization_from_map(_map)
        ...
        # Mapping requires all_maps_3d before _compute_Q_n (used by _Q_y_star)
        try:
            if self._get_problem_type() == 'mapping' and getattr(self, 'all_maps_3d', None) is None:
                self._initialize_all_maps_cache()
        except (NotImplementedError, AttributeError):
            pass  # Skip guard when _get_problem_type not available (e.g. base class)
    ...
    @staticmethod
    def _wrap_to_pi(phi: _numpy.ndarray) -> _numpy.ndarray:
        """Wrap angles to [-pi, pi)."""
        return (phi + _numpy.pi) % (2.0 * _numpy.pi) - _numpy.pi

    @staticmethod
    def build_range_bearing_per_landmark_quantization(
        obs_n: int,
        r_max: float,
        epsilon: float
    ) -> tuple[_numpy.ndarray, _numpy.ndarray]:
        """
        Build per-landmark representative points and rectangular (r,phi) bounds.

        This uses Euclidean (action-like disk) quantization only.

        Returns:
            points: (K, 2) as [r, phi]
            bounds: (K, 2, 2) with rows [r_bounds, phi_bounds]
        """
        # Euclidean action-like quantization on a disk, then convert to [r,phi].
        oq = ActionQuantizer(r_max, obs_n)
        xy = oq.U.get() if hasattr(oq.U, 'get') else _numpy.asarray(oq.U)
        rr = _numpy.linalg.norm(xy, axis=1)
        pp = _numpy.arctan2(xy[:, 1], xy[:, 0])
        per_landmark_points = _numpy.stack([rr, pp], axis=1)

        # Keep axis-aligned (r,phi) rectangles for compatibility with Q_n integration code.
        dr = (r_max - epsilon) / obs_n
        dphi = 2.0 * _numpy.pi / obs_n
        r_bounds = _numpy.stack([rr - dr / 2.0, rr + dr / 2.0], axis=1)
        r_bounds[:, 0] = _numpy.clip(r_bounds[:, 0], epsilon, r_max)
        r_bounds[:, 1] = _numpy.clip(r_bounds[:, 1], epsilon, r_max)
        phi_bounds = _numpy.stack([pp - dphi / 2.0, pp + dphi / 2.0], axis=1)
        phi_bounds = BaseBeliefMDP_n._wrap_to_pi(phi_bounds)
        per_landmark_bounds = _numpy.stack([r_bounds, phi_bounds], axis=1)
        return per_landmark_points, per_landmark_bounds

    def _configure_observation_quantization_from_map(self, map_obj=None):
        """Build Y_n and Y_bounds using current obs_n (Euclidean quantization)."""
        map_obj = map_obj or getattr(self, 'map', None)
        mode = 'euclidean'
        ...
        if hasattr(self, "sensor") and isinstance(self.sensor, LIDAR):
            ...
            self.Y_n = self.YQ.Y_n
            self.Y_bounds = self.YQ.get_bounds()
            return

        if hasattr(self, "sensor") and isinstance(self.sensor, RangeBearingSensor):
            num_landmarks = self._get_landmark_count(map_obj)
            if num_landmarks is None:
                raise ValueError("RangeBearingSensor requires LandmarkMap with num_candidate_landmarks")

            per_landmark_points, per_landmark_bounds = self.build_range_bearing_per_landmark_quantization(
                obs_n=self.obs_n,
                r_max=float(self.sensor.r_max),
                epsilon=float(self.sensor.epsilon)
            )
    ...
    def configure_observation_quantization(
        self,
        obs_n: int,
        ...
        self._configure_observation_quantization_from_map(getattr(self, 'map', None))
    ...
    def _get_landmark_count(self, map_obj=None):
        ...
        Prefer num_landmarks, then max_landmarks, then num_candidate_landmarks.
        """
        map_obj = map_obj or getattr(self, 'map', None)
        if map_obj is None:
            return None
    ...
    def _get_all_maps_cache_path(self) -> Path:
        ...
        if hasattr(self.map, 'occupancy_map'):
            # Occupancy grid map (LidarGridMapVec and relatives)
            filename = f"all_maps_ogm_H{self.map_H}_W{self.map_W}.npz"
        elif hasattr(self.map, 'n_cells'):
            # Position-quantized landmark map (LandmarkMap with n×n grid)
            L = self.map.num_candidate_landmarks
            K = self.map.n_cells  # n² cells per landmark
            fp = getattr(self.map, 'get_cache_fingerprint', lambda: ())()
            fp_hash = hashlib.md5(str(fp).encode()).hexdigest()[:8] if fp else ""
            filename = f"all_maps_landmark_L{L}_K{K}_{fp_hash}.npz" if fp_hash else f"all_maps_landmark_L{L}_K{K}.npz"
        else:
            # OrderedLandmarkMap or future map types
            filename = f"all_maps_H{self.map_H}_W{self.map_W}.npz"
        return cache_dir / filename

    def _initialize_all_maps_cache(self) -> None:
        """
        Centralize all_maps precompute decision/load logic.
        Sets both self.all_maps_3d and self.map_bits_array, or both None.
        - Occupancy grid maps: precompute only when map_H*map_W <= 16
        - Non-occupancy maps (landmark/etc): precompute only when len_M <= 65536
        """
        if hasattr(self.map, 'occupancy_map'):
    ...
    def _get_Q_cache_path(self) -> Path:
        ...
        metadata = {
            ...
            'n_map': n_map,
            'obs_n': self.obs_n,
            'obs_quantization_mode': 'euclidean',
            'state_bounds': self.state_bounds.flatten().tolist(),
            'max_val': max_val,
            'dt': float(self.motion_model.dt),
            'map_shape': (self.map_H, self.map_W),
            ...
            'kernel_version': 'v3-rb-detection-taper',
        }
        # LandmarkMap: include workspace grid so Q_n is invalidated when grid changes
        if hasattr(self.map, 'get_cache_fingerprint'):
            metadata['map_cache_fingerprint'] = self.map.get_cache_fingerprint()
        ...
        if isinstance(self.sensor, LIDAR):
            ...
            metadata['sensor_sigma_r'] = float(self.sensor.sigma_r)
            metadata['sensor_sigma_phi'] = float(self.sensor.sigma_phi)
            metadata['sensor_r0'] = float(self.sensor.r0)
            metadata['sensor_r1'] = float(self.sensor.r1)
        ...
        filename = f"Q_n_n{self.n}{nmap_part}_obs{self.obs_n}_map{self.map_H}x{self.map_W}_{sensor_suffix}_{metadata_hash}.npz"
        return cache_dir / filename
    ...
    def _load_Q_n(self, cache_path: Path) -> bool:
        ...
        try:
            ...
            if hasattr(self.map, 'get_cache_fingerprint'):
                expected_metadata['map_cache_fingerprint'] = self.map.get_cache_fingerprint()

            # Add sensor-specific metadata
            if isinstance(self.sensor, LIDAR):
                expected_metadata['sensor_B'] = int(self.sensor.B)
            elif isinstance(self.sensor, RangeBearingSensor):
                expected_metadata['sensor_sigma_r'] = float(self.sensor.sigma_r)
                expected_metadata['sensor_sigma_phi'] = float(self.sensor.sigma_phi)
                expected_metadata['sensor_r0'] = float(self.sensor.r0)
                expected_metadata['sensor_r1'] = float(self.sensor.r1)
                num_landmarks = self._get_landmark_count()
                if num_landmarks is not None:
            ...
            for key, expected_value in expected_metadata.items():
                ...
                if key == 'map_cache_fingerprint':
                    # Tuple fingerprint: normalize for comparison (saved as array by savez)
                    data_val = tuple(data_value.tolist()) if hasattr(data_value, 'tolist') else tuple(data_value)
                    if data_val != expected_value:
                        print(f"  Cache mismatch: '{key}' (map grid changed)")
                        return False
                    continue
                if isinstance(expected_value, _numpy.ndarray):
                    if not _numpy.array_equal(data_value, expected_value):
    ...
    def _compute_Q_n(self) -> np.ndarray:
        ...
        for k in tqdm(range(m_y), desc="Computing Q_n"):
            ...
            if hasattr(self, "sensor") and isinstance(self.sensor, RangeBearingSensor):
                ...
                p_det = _numpy.zeros_like(r_star, dtype=_numpy.float64)
                if _numpy.any(visible_mask):
                    r_vis = r_star[visible_mask]

                    eta_in = _numpy.zeros_like(r_vis, dtype=_numpy.float64)
                    in_mid = (r_vis > eps) & (r_vis < r0)
                    eta_in[in_mid] = 3.0 * ((r_vis[in_mid] - eps) / (r0 - eps))**2 - \
                        2.0 * ((r_vis[in_mid] - eps) / (r0 - eps))**3
                    eta_in = _numpy.where(r_vis >= r0, 1.0, eta_in)

                    eta_out = _numpy.zeros_like(r_vis, dtype=_numpy.float64)
                    out_mid = (r_vis > r1) & (r_vis < r_max_sensor)
                    eta_out[out_mid] = 3.0 * ((r_vis[out_mid] - r1) / (r_max_sensor - r1))**2 - \
                        2.0 * ((r_vis[out_mid] - r1) / (r_max_sensor - r1))**3
                    eta_out = _numpy.where(r_vis >= r_max_sensor, 1.0, eta_out)

                    p_det_vis = _numpy.clip(eta_in * (1.0 - eta_out), 0.0, 1.0)
                    p_det[visible_mask] = p_det_vis
                ...
                per_landmark_prob_obs = _numpy.asarray(p_det * r_prob * bearing_prob)
                per_landmark_prob = _numpy.asarray(per_landmark_prob_obs)
                ...
                if getattr(self, 'cov_y', None) is not None:
                    cov_np = self.cov_y.get() if hasattr(self.cov_y, 'get') else _numpy.asarray(self.cov_y)
                    sigma_vec = _numpy.sqrt(_numpy.clip(_numpy.diag(cov_np), 1e-16, None))
                else:
    ...
    def _compute_Q_batch_for_obs_indices(self, obs_indices: np.ndarray) -> np.ndarray:
        ...
        if hasattr(self, "sensor") and isinstance(self.sensor, RangeBearingSensor):
            ...
            for i, k in enumerate(obs_indices):
                ...
                sentinel_mask = _numpy.isnan(Y_n_np[k]).reshape(num_landmarks, 2).all(axis=1)
                per_landmark_prob = _numpy.where(
                    sentinel_mask.reshape(expand_shape),
                    1.0 - p_det,
                    per_landmark_prob
                )
    ...
    @abstractmethod
    def F_batch_log(self, B, *args, Y_batch):
        """
        Batched filter update equation in log-space. Must be implemented by subclasses.

        Note: Signature varies by problem type:
        - SLAM/Localization: F_batch_log(B, u, Y_batch)
        - Mapping: F_batch_log(B, x_next, Y_batch) - uses known pose instead of action
        """
        raise NotImplementedError

    def H(self, B: np.ndarray) -> np.ndarray:
        """
        Compute entropy of belief distribution(s): H(B) = -Σ B * log(B).

        Vectorized to handle batches of beliefs. Used by r_information_gain.

        Args:
            B: belief distribution(s), shape (..., m_n, len_M) for SLAM, (..., m_n,) for Localization, (..., len_M,) for Mapping
        Returns:
            entropy: entropy value(s), scalar if single belief, array if batched
        """
        problem_type = self._get_problem_type()
        is_single = (B.ndim == 1) or (problem_type == 'slam' and B.ndim == 2)
        ...
        # Flatten then share Shannon entropy with finite_memory via classes.costs.
        from ..classes.costs import shannon_entropy_rows
        H_vals = shannon_entropy_rows(B_flat)
        return H_vals[0] if is_single else H_vals

    def r_information_gain(self, B_batch: np.ndarray) -> np.ndarray:
        """
        Stage-wise entropy reward: r_information_gain(b_t) = H(b_t).

        Args:
            B_batch: beliefs (n_B, ...) or a single belief
        Returns:
            r_batch: (n_B,) where r_batch[i] = H(B_batch[i])
        """
        H_B_batch = self.H(B_batch)
        return np.atleast_1d(H_B_batch)

    @abstractmethod
    def ρ_n(self, B_batch: np.ndarray, *args: Any) -> np.ndarray:
        """
        Expected cost function for belief-MDP. Must be implemented by subclasses.

        Note: Signature varies by problem type:
        - SLAM/Localization: ρ_n(B, u) = r_exploration(B) + c_effort(u)
        - Mapping: ρ_n(B, x_current, u) - may need current pose for cost computation
        """
        raise NotImplementedError


    @abstractmethod
    def η_n(self, B_new: np.ndarray, B: np.ndarray, *args) -> float:
        """
        Belief transition probability. Must be implemented by subclasses.

        Note: Signature varies by problem type:
        - SLAM/Localization: η_n(B_new, B, u, ...)
        - Mapping: η_n(B_new, B, x_current, u, x_next, ...) - computes joint probability η(B', x_next | b_t, u)
          where b_t = (x_t, B_t) is the augmented state, and x_next is given as input (not sampled).
          Implements: η(B', x_next | b_t, u) = T(x_next | x_t, u_t) * ∫_Y δ_{F(B_t, x_next, y)}(B') * H(dy | B_t, x_next)
        """
        raise NotImplementedError
...
class BeliefMDP_n_SLAM(SLAM_POMDP, BaseBeliefMDP_n):
    ...
    Belief space: B(x, m) - shape (m_n, len_M)
    ...
    def __init__(
        ...
        SLAM_POMDP.__init__(self, motion_model, measurement_model, obstacles, _map, sigma_w, sigma_v, cov_y=cov_y)
        ...
        self._init_common_slam(
            n,
            motion_model,
            _map,
            obs_n=obs_n if obs_n is not None else n,
            action_n=action_n
        )
    ...
    def _Q_y_star(self):
        ...
        y_star_result = self.ray_casting(X_in, self.all_maps_3d)
        # Convert CuPy array to NumPy if needed
    ...
    def flatten_belief(self, B_2d: np.ndarray) -> np.ndarray:
        """
        Convert belief from (m_n, len_M) to (N_n,) where N_n = m_n * len_M.

        Flattening order: row-major (C-order), so beliefs are indexed as:
        B_flat[i * len_M + j] = B_2d[i, j]

        Args:
            B_2d: belief (m_n, len_M)
        Returns:
            B_flat: belief (N_n,) where N_n = m_n * len_M
        """
        return B_2d.flatten(order='C')

    def unflatten_belief(self, B_flat: np.ndarray) -> np.ndarray:
        """
        ...
        Returns:
            B_2d: belief (m_n, len_M)
        """
        return B_flat.reshape((self.SQ.m_n, self.len_M), order='C')
    ...
    def F(self, B, u, y):
        ...
        Args:
            B: current belief (m_n, len_M)
            u: action (2,)
            y: observation (B,) or (1, B)
        Returns:
            B_new: updated belief (m_n, len_M) or list containing it
        """
        # Ensure y is 2D
        y = np.asarray(y)
        if y.ndim == 1:
        ...
        B_new_batch = self.F_batch_log(B, u, y)  # (1, m_n, len_M)
        return [B_new_batch[0]]  # Return as list for compatibility with test code

    def F_batch_log(self, B, u, Y_batch):
        """
        ...
        Returns:
            B_new_batch: updated beliefs (K, m_n, len_M)
        """
        if self.all_maps_3d is None:
            raise ValueError(
    ...
    def _compute_H_y_and_F(self, B: np.ndarray, u: np.ndarray):
        """
        ...
        Returns:
            ...
            B_all_batch: updated beliefs for all observations (m_y, m_n, len_M)
        """
        if self.Q_n is None:
        ...
        B_all_batch = np.exp(log_F_2d).reshape(m_y, *predicted.shape)  # (m_y, m_n, len_M)

        return H_y, B_all_batch

    def η_n(self, B_new: np.ndarray, B: np.ndarray, u: np.ndarray) -> float:
        """
        Compute transition probability η_n(B' | B, u) using finite observation quantization.
        ...
        With a finite quantized observation alphabet Y_n, we approximate:
            η_n(B' | B, u) = ∑_{y∈Y_n} 𝟙_{F(B,u,y) ≈ B'} · H({y} | B, u)

        Args:
            B_new: target belief, either (m_n, len_M) or (N_n,) where N_n = m_n * len_M
            B: current belief, either (m_n, len_M) or (N_n,)
            u: action (2,)
        Returns:
            float: probability of transitioning to B' from B under action u
        """
        # Normalize beliefs to 2D format if needed
        B_new_2d = B_new if B_new.ndim == 2 else self.unflatten_belief(B_new)
        B_2d = B if B.ndim == 2 else self.unflatten_belief(B)

        # Use shared helper to compute H_y and F
        H_y, B_all_batch = self._compute_H_y_and_F(B_2d, u)
        m_y = int(self.Y_n.shape[0])

        # 4) Vectorized distance computation: L2 norm between each B_k and B_new
        B_new_broadcast = np.broadcast_to(B_new_2d[np.newaxis, :, :], (m_y, *B_new_2d.shape))  # (m_y, m_n, len_M)
        B_diff = B_all_batch - B_new_broadcast  # (m_y, m_n, len_M)
        distances = np.linalg.norm(B_diff.reshape(m_y, -1), axis=1)  # (m_y,)
    ...
    def _prepare_observation_batch(self, Y_batch: np.ndarray) -> np.ndarray:
        ...
        if Y.ndim == 1:
            ...
            obs_dim = int(self.Y_n.shape[1]) if getattr(self, "Y_n", None) is not None and self.Y_n.ndim == 2 else None
            if obs_dim is not None and Y.shape[1] != obs_dim and Y.size == obs_dim:
                # Single structured observation (e.g., (n_landmarks, 2)) -> flatten to (1, B)
                Y = Y.reshape(1, -1)
        elif Y.ndim == 3:
    ...
    def _compute_H_y_and_F_batched(self, B_batch: np.ndarray, U_batch: np.ndarray):
        """
        Batched version of _compute_H_y_and_F for all belief-action pairs.

        Args:
            B_batch: beliefs (n_B, m_n, len_M)
            U_batch: actions (n_u, 2)
        Returns:
            H_y_batch: observation probabilities (n_B, n_u, m_y)
            B_all_batch: updated beliefs (n_B, n_u, m_y, m_n, len_M)
        """
        if self.Q_n is None:
            raise ValueError("Q_n must be precomputed. Ensure observation quantization is configured.")

        n_B, n_u = B_batch.shape[0], U_batch.shape[0]
        m_y = int(self.Y_n.shape[0])
        m_n, len_M = B_batch.shape[1], B_batch.shape[2]
        ...
        # T_mat: (m_n, m_n, n_u), B_batch: (n_B, m_n, len_M)
        # Extract T matrices for all actions: (m_n, m_n, n_u)
        T_mat_selected = self.T_mat[:, :, u_indices]  # (m_n, m_n, n_u)

        # Vectorized matrix multiplication: (m_n, m_n, n_u) @ (n_B, m_n, len_M) -> (n_B, n_u, m_n, len_M)
        # Using einsum: 'ijk,ilm->ijlm' where i=m_n, j=n_u, k=m_n, l=n_B, m=len_M
        # Actually: T_mat_selected: (m_n, m_n, n_u), B_batch: (n_B, m_n, len_M)
        # We want: for each (B_i, u_j): T_mat[:, :, u_j] @ B_i
        ...
        # Now: (n_u, m_n, m_n) @ (n_B, m_n, len_M) -> (n_B, n_u, m_n, len_M)
        predicted_batch = np.einsum('jkl,ilm->ijlm', T_mat_rearranged, B_batch)  # (n_B, n_u, m_n, len_M)

        log_predicted_batch = np.log(np.maximum(predicted_batch, 1e-300))  # (n_B, n_u, m_n, len_M)

        # 2) Compute H_y for all (belief, action, observation) combinations
        # Q_n: (m_y, m_n, len_M), log_predicted_batch: (n_B, n_u, m_n, len_M)
        log_Q_n = np.log(np.maximum(self.Q_n, 1e-300))  # (m_y, m_n, len_M)

        # Broadcast: (1, 1, m_y, m_n, len_M) + (n_B, n_u, 1, m_n, len_M) -> (n_B, n_u, m_y, m_n, len_M)
        log_numerator = log_Q_n[np.newaxis, np.newaxis, :, :, :] + \
            log_predicted_batch[:, :, np.newaxis, :, :]  # (n_B, n_u, m_y, m_n, len_M)

        # H_y = sum(Q_n * predicted) over (x', m) - compute in log space
        log_numerator_flat = log_numerator.reshape(n_B, n_u, m_y, -1)  # (n_B, n_u, m_y, m_n * len_M)
        log_H_y_batch = logsumexp(log_numerator_flat, axis=3)  # (n_B, n_u, m_y)
        H_y_batch = np.exp(log_H_y_batch)  # (n_B, n_u, m_y)

        # Normalize H_y for each (belief, action) pair
        total_H = np.sum(H_y_batch, axis=2, keepdims=True)  # (n_B, n_u, 1)
        H_y_batch = np.where(total_H > 0, H_y_batch / total_H, H_y_batch)  # (n_B, n_u, m_y)

        # 3) Compute F for all combinations
        # F = (Q_n * predicted) / H_y, so log_F = log_numerator - log_H_y
        log_F_flat = log_numerator_flat - log_H_y_batch[:, :, :, np.newaxis]  # (n_B, n_u, m_y, m_n * len_M)
        B_all_batch = np.exp(log_F_flat).reshape(n_B, n_u, m_y, m_n, len_M)  # (n_B, n_u, m_y, m_n, len_M)

        return H_y_batch, B_all_batch

    def _get_pose_distance_matrix(self) -> np.ndarray:
        """Pairwise Euclidean distances between pose states: D[i,j] = ||x_i - x_j||."""
        if getattr(self, "_pose_distance_matrix", None) is None:
            X = self.SQ.X_n  # (m_n, state_dim)
            diff = X[:, np.newaxis, :] - X[np.newaxis, :, :]
            self._pose_distance_matrix = np.linalg.norm(diff, axis=2)
        return self._pose_distance_matrix

    def rao_entropy(self, B_batch: np.ndarray) -> np.ndarray:
        """
        Rao entropy for SLAM: RE(b) = ∫∫ d(x,x') b(dx) b(dx') applied uniformly to
        map and pose marginals, each with its respective metric.
        - Map: d_M (map metric)
        - Pose: Euclidean ||x - x'||
        """
        if B_batch.ndim == 2:
            B_batch = B_batch[np.newaxis, :, :]
        # Map marginal: RE(b_map) = b_map^T D_M b_map
        b_map_batch = np.sum(B_batch, axis=1)  # (n_B, len_M)
        map_cost = BeliefMDP_n_Mapping.rao_entropy(self, b_map_batch)

        # Pose marginal: RE(b_pose) = b_pose^T D_pose b_pose with Euclidean metric
        b_pose_batch = np.sum(B_batch, axis=2)  # (n_B, m_n)
        D_pose = self._get_pose_distance_matrix()  # (m_n, m_n)
        pose_cost = np.einsum("bi,ij,bj->b", b_pose_batch, D_pose, b_pose_batch)

        return map_cost + pose_cost

    def ρ_n(self, B_batch: np.ndarray, U_batch: np.ndarray) -> np.ndarray:
        from ..classes.costs import compose_rho
        if B_batch.ndim == 2:
            B_batch = B_batch[np.newaxis, :, :]
        if U_batch.ndim == 1:
            U_batch = U_batch[np.newaxis, :]
        return compose_rho(self.r_exploration(B_batch), self.c_effort(U_batch))
...
class BeliefMDP_n_Localization(Localization_POMDP, BaseBeliefMDP_n):
    ...
    def __init__(
        ...
        # Initialize POMDP parent first
        Localization_POMDP.__init__(self, motion_model, measurement_model, obstacles, _map, sigma_w, sigma_v, cov_y=cov_y)
        ...
        # Localization uses information gain only for exploration cost
        self.exploration_type = 'information gain'
    ...
    def F(self, B, u, y):
        ...
        Args:
            B: current belief (m_n,)
            u: action (2,)
            y: observation (B,) or (1, B)
        Returns:
            B_new: updated belief (m_n,) or list containing it
        """
        # Ensure y is 2D
        y = np.asarray(y)
        if y.ndim == 1:
        ...
        # Use F_batch_log and return first result
        B_new_batch = self.F_batch_log(B, u, y)  # (1, m_n)
        return [B_new_batch[0]]  # Return as list for compatibility with test code
    ...
    def F_batch_log(self, B, u, Y_batch):
        ...
        Args:
            ...
            Y_batch: observations (K, B)
        Returns:
            B_new_batch: updated beliefs (K, m_n)
        """
        if self.known_map_obstacle_segments is None:
            raise ValueError("Known map obstacle segments not set. Call set_known_map() first.")
    ...
    def ρ_n(self, B_batch: np.ndarray, U_batch: np.ndarray) -> np.ndarray:
        """
        Cost for localization: ρ_n(B, u) = r_exploration(B) + c_effort(u).
        Uses information gain only for exploration (entropy H(B)).
        """
        from ..classes.costs import compose_rho
        if B_batch.ndim == 1:
            B_batch = B_batch[np.newaxis, :]
        if U_batch.ndim == 1:
            U_batch = U_batch[np.newaxis, :]
        return compose_rho(self.r_information_gain(B_batch), self.c_effort(U_batch))
    ...
    def η_n(self, B_new: np.ndarray, B: np.ndarray, u: np.ndarray) -> float:
        ...
        where H({y_k} | B, u) is the probability mass of the k-th observation
        cell, and F is evaluated at the corresponding representative y_k.

        Args:
            B_new: target belief (m_n,)
            B: current belief (m_n,)
            u: action (2,)
        Returns:
            float: probability of transitioning to B' from B under action u
        ...
        B_new_1d = B_new.flatten() if B_new.ndim > 1 else B_new
        B_1d = B.flatten() if B.ndim > 1 else B
        ...
        B_all_batch = np.exp(log_F)  # (m_y, m_n)

        # 4) Vectorized distance computation
        B_new_broadcast = np.broadcast_to(B_new_1d[np.newaxis, :], (m_y, len(B_new_1d)))  # (m_y, m_n)
        B_diff = B_all_batch - B_new_broadcast  # (m_y, m_n)
        distances = np.linalg.norm(B_diff, axis=1)  # (m_y,)
...
class BeliefMDP_n_Mapping(Mapping_POMDP, BaseBeliefMDP_n):
    ...
    def __init__(
        ...
        # Initialize POMDP parent first
        Mapping_POMDP.__init__(self, motion_model, measurement_model, obstacles, _map, sigma_w, sigma_v, cov_y=cov_y)
    ...
    def rao_entropy(self, B_batch: np.ndarray) -> np.ndarray:
        ...
        return np.einsum("bi,ij,bj->b", B_batch, D, B_batch)

    def r_exploration(self, B_batch: np.ndarray) -> np.ndarray:
        """Exploration cost: Shannon entropy or Rao entropy (Wasserstein), depending on exploration_type."""
        if self.exploration_type == 'information gain':
            return self.r_information_gain(B_batch)
        elif self.exploration_type == 'wasserstein distance':
            return self.rao_entropy(B_batch)
    ...
    def ρ_n(self, B_batch: np.ndarray, *args: Any) -> np.ndarray:
        """
        Stage cost for mapping: ρ_n(B, u) = r_exploration(B) + c_effort(u).
        Satisfies abstract method; for quantized MDP, BeliefMDP_n_M uses precomputed c_n_M instead.
        """
        from ..classes.costs import compose_rho
        U_batch = np.asarray(args[-1]) if len(args) >= 1 else np.array([[0.0, 0.0]])
        if U_batch.ndim == 1:
            U_batch = U_batch[np.newaxis, :]
        B_batch = np.atleast_2d(B_batch)
        return compose_rho(self.r_exploration(B_batch), self.c_effort(U_batch))
        ...
        if hasattr(self, "sensor") and isinstance(self.sensor, RangeBearingSensor):
            ...
            for i in range(Y_batch_np.shape[0]):
                if _numpy.any(nan_mask[i]):
                    ...
                    dist = _numpy.where(_numpy.isnan(dist), _numpy.inf, dist)
                    obs_indices[i] = int(_numpy.argmin(dist))
            return np.asarray(obs_indices)
        return np.argmin(cdist(Y_batch, self.Y_n), axis=1)

    ##########################################################
    ### 1. Filter Update and Observation Functions ######
    def F(self, B, x_next, y):
        """
        Filter update for a single observation.
        ...
        Args:
            B: current belief (len_M,)
            x_next: known next pose (state_dim,) or (2,)
            y: observation (B,) or (1, B)
        Returns:
            B_new: updated belief (len_M,) or list containing it
        """
        # Ensure y is 2D
        y = np.asarray(y)
        if y.ndim == 1:
        ...
        # Use F_batch_log and return first result
        B_new_batch = self.F_batch_log(B, x_next, y)  # (1, len_M)
        return [B_new_batch[0]]  # Return as list for compatibility with test code

    def F_batch_log(self, B, x_next, Y_batch):
        """
        Batched filter update in log-space for numerical stability.

        Reuses H computation: F = (Q * B) / H, where H = sum(Q * B).
        ...
        Args:
            ...
            Y_batch: observations (K, B)
        Returns:
            B_new_batch: updated beliefs (K, len_M)
        """
        if self.all_maps_3d is None:
            raise ValueError(
                "F_batch_log requires all_maps_3d to be precomputed. "
        ...
        # Compute numerator: Q * B (in log space)
        log_numerator = log_Q_batch + np.broadcast_to(log_B[np.newaxis, :], (Y_batch.shape[0], len(log_B)))

        # Compute H (normalization constant) = sum(Q * B) over maps
        log_H = logsumexp(log_numerator, axis=1, keepdims=True)  # (K, 1)
    ...
    def η_n(self, B_new: np.ndarray, B: np.ndarray, x_current: np.ndarray, u: np.ndarray, x_next: np.ndarray) -> float:
        """
        Compute joint transition probability η_n(B', x_next | b_t, u) using finite observation quantization.

        Implements the joint probability:
        η(B', x_next | b_t, u) = T(x_next | x_t, u_t) * ∑_{y∈Y_n} 𝟙_{F(B_t, x_next, y) ≈ B'} · H({y} | B_t, x_next)

        where b_t = (x_t, B_t) is the augmented state, and x_next is given as input.

        Args:
            B_new: target belief (len_M,)
            B: current belief (len_M,)
            x_current: current state (state_dim,) - part of augmented state b_t = (x_t, B_t)
            u: action (2,)
            x_next: next state (state_dim,) - given as input, not sampled
        Returns:
            float: joint probability of (B', x_next) from augmented state (x_t, B_t) under action u
        """
        if self.Q_n is None:
            raise ValueError("Q_n must be precomputed. Ensure observation quantization is configured.")
        ...
        # Step 2: Compute H for ALL observations at once: H_y = sum(Q_slice * B) over maps
        m_y = int(self.Y_n.shape[0])
        ...
        log_B = np.log(np.maximum(B_1d, 1e-300))  # (len_M,)
        log_numerator = log_Q_slice + log_B[np.newaxis, :]  # (m_y, len_M)

        # H_y = sum(Q_slice * B) over maps - compute in log space for stability
        log_H_y = logsumexp(log_numerator, axis=1)  # (m_y,)
        H_y = np.exp(log_H_y)  # (m_y,)
        ...
        # Step 3: Compute F for ALL observations directly from Q_slice and B (reusing H_y computation!)
        # F = (Q_slice * B) / H_y, so log_F = log_numerator - log_H_y
        log_F = log_numerator - log_H_y[:, np.newaxis]  # (m_y, len_M)
        B_all_batch = np.exp(log_F)  # (m_y, len_M)

        # Step 4: Vectorized distance computation
        B_new_broadcast = np.broadcast_to(B_new_1d[np.newaxis, :], (m_y, len(B_new_1d)))  # (m_y, len_M)
        B_diff = B_all_batch - B_new_broadcast  # (m_y, len_M)
        distances = np.linalg.norm(B_diff, axis=1)  # (m_y,)
        ...
        # Step 7: Return joint probability = T(x_next | x_current, u) * P(B' ≈ B_new | x_next)
